...
public class HolidayController {
    ...
    private void addHoliday() {
        try {
            ...
            // Vérifier si les dates chevauchent avec un congé existant
            if (dao.hasOverlappingHoliday(employeeName, startDate, endDate)) {
                throw new IllegalArgumentException("Chevauchement détecté avec un congé existant pour cet employé.");
            }
            ...
            // Vérifier le total des jours pris cette année
    ...
    private void modifyHoliday() {
        try {
            ...
            if (actionCommand != null && !actionCommand.trim().isEmpty()) {
                ...
                // Vérifier si les dates chevauchent avec un congé existant, en excluant le congé en cours de modification
                List<Holiday> holidays = dao.listAll();
                boolean hasOverlap = holidays.stream()
                    .anyMatch(h -> h.getEmployeeName().equals(employeeName) && h.getId() != id &&
                        ((LocalDate.parse(startDate).isBefore(LocalDate.parse(h.getEndDate())) && LocalDate.parse(endDate).isAfter(LocalDate.parse(h.getStartDate()))) ||
                        (LocalDate.parse(startDate).isEqual(LocalDate.parse(h.getStartDate())) || LocalDate.parse(endDate).isEqual(LocalDate.parse(h.getEndDate())))));
